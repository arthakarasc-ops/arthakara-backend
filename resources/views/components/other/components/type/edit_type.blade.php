...
<div class="max-w-xl mx-auto">
    <div class="mb-8">
        <a href="{{ route('type.get') }}" class="text-slate-500 hover:text-cyan-600 transition-colors flex items-center gap-2 mb-4 group">
    ...
    <form action="{{ route('type.update', ['typeId' => $type->id]) }}" method="POST" class="bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 shadow-sm space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="flex gap-4">
            <a href="{{ route('type.get') }}" class="flex-1 bg-slate-100 text-slate-600 font-bold py-4 rounded-2xl text-center hover:bg-slate-200 transition-all">Cancel</a>
