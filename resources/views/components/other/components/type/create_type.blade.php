...
<div class="max-w-xl mx-auto">
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-slate-900">New Type</h1>
        <p class="text-slate-500">Add a new category type for your products.</p>
    </div>
    ...
    <form id="type-form" action="{{ route('types.create') }}" method="POST" class="bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 shadow-sm space-y-6">
        @csrf

        <div>
            <label class="block text-slate-700 font-semibold mb-2">Type Name</label>
            <input type="text" name="name" value="{{ old('name') }}"
                class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all outline-none @error('name') border-red-500 @enderror" required placeholder="e.g. Reed Diffuser">
            @error('name')
