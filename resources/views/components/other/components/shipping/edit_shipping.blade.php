...
<div class="max-w-2xl mx-auto">
    <div class="mb-8 text-center sm:text-left">
        ...
        <h1 class="text-3xl font-bold text-slate-900">Edit Shipping</h1>
        <p class="text-slate-500">Update configuration for <span class="text-cyan-600">"{{ $shipping->name }}"</span>.</p>
    </div>
    ...
    <form action="{{ route('shippings.update', $shipping->id) }}" method="POST" class="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm space-y-6">
        ...
        <div class="space-y-2">
            ...
            <input type="text" name="name" id="name" value="{{ old('name', $shipping->name) }}"
        ...
        <div class="space-y-2">
            ...
            <div class="relative">
                ...
                <input type="number" name="price" id="price" value="{{ old('price', $shipping->price) }}"
        ...
        <div class="space-y-2">
            ...
            <textarea name="description" id="description" rows="4"
                      class="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/10 transition-all">{{ old('description', $shipping->description) }}</textarea>
        </div>

        <div class="pt-4 flex flex-col sm:flex-row gap-4">
            <a href="{{ route('shipping.index') }}" class="flex-1 text-center py-4 rounded-2xl font-bold text-slate-500 bg-slate-100 hover:bg-slate-200 transition-all">Cancel</a>
